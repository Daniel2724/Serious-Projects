...
public class School_Grades_2 {
	...
	public static void main(String[] args) {
		...
		while (classes > 0) // it is to stop the loop when all the classes got there grades
		{
			...
			{
				System.out.println("Give me the grades for class " + classes + " ( To stop write '-1' )"); // asking for
																											// a grade
				grade = reader.nextInt(); // getting the grade
				if (grade != -1) // checking if the input is -1
				{
					if (grade <= 100) // if the grade is more then 100 the program skip to the "else" and S.O.P that
										// this grade is too big
						...
						if (grade < 60) // if the grade is less then 60 the program and 1 ((looser:-)) to the int f
						{
							...
							f = f + 1;
						} else // just in case every thing is right the program simply add 1 to the count of
								// the sudents (int stu) and add the grade to the sum of the grades (double
								// sumGrades)
						{
							stu = stu + 1;
							sumGrades = sumGrades + grade;
			...
			System.out.println("The avarage of class " + classes + " is " + avg); // print the average of the specific
																					// class in the school
			System.out.println(f + " studenst in this class has faild"); // print how much students in this specific
																			// class failed

			sumT = sumT + sumGrades; // add to the total sum of the grades in all the classes (double sumT) the grade
										// of the last class (double sumGrades)
			stuT = stuT + stu; // add to the total amount of students in the school (int stuT), the amount of
								// the students in the last class
			classes = classes - 1;
			grade = 0; // zeroing the grade parameter, `couse now (int) grade is -1 (from the last
						// time) and now to make the next while loop work the (int) grade need to be {
						// != } to -1 so it was zeroed !
			fT = fT + f;
		}
		avgT = sumT / stuT;
